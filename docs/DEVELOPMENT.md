...
8765 被别的软件占了的话（Windows 上常见的是百度输入法），可以改：菜单栏
「服务端口…」填新端口（留空恢复默认，退出重开生效），或启动时用
`AICLOCK_PORT=8766`（LaunchAgent 写进 `EnvironmentVariables`）、`--port 8766`。
优先级：`--port` > `AICLOCK_PORT` > 存的设置 > 8765，实际端口打在启动日志里。端口
被占用时 app 会弹窗提示，不再默默不工作。

改完端口后点一次「把本机设为设备桥接」（或在设备网页把 Bridge host 改成
`<本机IP>:8766`），固件本来就按 `host:port` 存，不用重刷；hooks 命令里的
`127.0.0.1:8765/event` 也要跟着改。
