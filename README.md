...
此外还支持**增强天气页**、**名人名言页**和**「此刻」综合页**。天气页包含当前/体感/最高/最低温、湿度、风力、空气质量、降水概率和未来三天预报；名言页展示中文一言，并在断网时沿用最近一次成功缓存；「此刻」把名言、天气和 Codex 周额度集中到一屏，适合长期固定显示。
...
名言由 Mac 桥接程序从一言的文学、哲学、诗词、原创、其他和影视分类获取中文内容，无需 API Key，并将接口返回长度限制为 45 字；约每 5 分钟自动换一句并避免近期重复。本机会缓存最新中文和最近 20 条历史；升级后旧英文缓存会被忽略并清理。网络失败时保留上次中文结果，超过 30 分钟会标记为陈旧，但仍可继续显示。名言功能需要同时更新 Mac 桥接程序与 ESP8266 固件。自动显示设置总会先保存在 Mac；如果能连接到低于 v0.4.13 的旧固件，保存后会提示升级，而不会把本地保存误报成设备已同步。
